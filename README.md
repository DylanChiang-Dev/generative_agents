...
![三人在 Hobbs Cafe 會面 - 2025年2月14日 08:43:30](rag_test_002_meeting.png)
...
> [!NOTE]
> **關於每日計劃生成**：原版 Generative Agents 會透過 LLM 動態生成每位角色的每日行程安排（Daily Schedule）。由於算力成本考量，本實驗將每日計劃改為**預先編寫**並存入 `bootstrap_memory/scratch.json` 的 `f_daily_schedule` 字段，跳過 LLM 生成步驟。這樣可以精確控制角色的行為軌跡，確保三人在指定時間地點會面，同時大幅降低 API 調用成本。
